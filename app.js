...
const GRANATE = '#a01e34', GRANATE2 = '#7a1428', ORO = '#c9a227', TEAL = '#0e7c86';
...
async function boot() {
  DATA.presu = await load('presupuesto-sanmarcos.json');
  DATA.biblio = await load('bibliometria.json');
  DATA.prov = await load('proveedores-sanmarcos.json');
  DATA.plan = await load('planilla-sanmarcos.json');
  render();
}
...
function renderGasto() {
  ...
  if (note) note.innerHTML = `Ejercicio ${d.anio} (cerrado). ` +
    ...
    `Fuente: MEF/SIAF, pliego 510 (San Marcos).`;
}
...
function renderProv() {
  ...
  if (!V?.proveedores?.length) {
    w.innerHTML = `<div class="soon"><h3>🔎 En construcción</h3><p style="margin:0;color:var(--muted)">Descargando y cruzando las contrataciones de San Marcos (OECE/CONOSCE) para listar cada proveedor, su RUC, el monto y sus dueños/representantes.</p></div>`;
    return;
  }
  const t = V.totales || {};
  // KPIs
  ...
  w.innerHTML = `<div class="card">
    <h3>Proveedores de San Marcos · ${fmtN(V.proveedores.length)}</h3>
...
function renderInv() {
  ...
  const peers = [['uni', 'San Marcos'], ['pucp', 'PUCP'], ['cayetano', 'Cayetano'], ['unalm', 'La Molina'], ['upc', 'UPC'], ['uni_ing', 'UNI']]
    .map(([k, n]) => DATA.biblio[k] ? { n, v: DATA.biblio[k].works } : null).filter(Boolean).sort((a, b) => b.v - a.v);
  new Chart(cPares, {
    type: 'bar', data: { labels: peers.map(x => x.n), datasets: [{ label: 'Publicaciones', data: peers.map(x => x.v), backgroundColor: peers.map(x => x.n === 'San Marcos' ? GRANATE : ORO) }] },
    options: opts({ indexAxis: 'y', plugins: { legend: { display: false } } })
  });
  document.getElementById('invNote').textContent = `San Marcos: ${fmtN(b.works)} publicaciones, ${fmtN(b.cited)} citas, h-index ${b.h_index} (OpenAlex, ${DATA.biblio._meta?.extraido || '2026'}). Mayor VOLUMEN de publicaciones del país; el impacto por publicación (citas/artículo) es más bajo que el de sus pares y hay un pico atípico en 2014 (dato de la fuente) — interpretar con cautela.`;
}
...
function renderPlan() {
  ...
  if (!P?.personas?.length) {
    w.innerHTML = `<div class="soon"><h3>🔎 En construcción</h3><p style="margin:0;color:var(--muted)">Cruzando la planilla de San Marcos (docentes, funcionarios y personal, con sus remuneraciones) desde datos públicos del Estado (AIRHSP / portal de transparencia). Este portal <strong>solo muestra personal — nunca nombres de estudiantes</strong>.</p></div>`;
    return;
  }
...
function buildCtx() {
  ...
  let c = 'Eres el asistente del Observatorio San Marcos, portal ciudadano de transparencia de la Universidad Nacional Mayor de San Marcos (UNMSM, pliego 510), con datos públicos (MEF, OECE, OpenAlex). Responde corto, en español, solo sobre San Marcos y sus datos. No inventes cifras.';
